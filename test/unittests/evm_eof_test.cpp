...
TEST_P(evm, eof1_codecopy_full)
{
    ...
    EXPECT_EQ(bytes_view(result.output_data, result.output_size),
        "ef0001010004020001000c0300000000800003601f6000600039601f6000f3"_hex);
    ...
    EXPECT_STATUS(EVMC_SUCCESS);
    EXPECT_EQ(bytes_view(result.output_data, result.output_size),
        "ef0001010004020001000c03000400008000036023600060003960236000f3deadbeef"_hex);
}
...
TEST_P(evm, eof1_codecopy_out_of_bounds)
{
    ...
    EXPECT_STATUS(EVMC_SUCCESS);
    EXPECT_EQ(bytes_view(result.output_data, result.output_size),
        "ef0001010004020001000c03000000008000036023600060003960236000f300000000"_hex);
    ...
    EXPECT_STATUS(EVMC_SUCCESS);
    EXPECT_EQ(bytes_view(result.output_data, result.output_size),
        "ef0001010004020001000c03000400008000036027600060003960276000f3deadbeef00000000"_hex);
}

TEST_P(evm, eof_data_only_contract)
{
    rev = EVMC_CANCUN;
    auto code = "EF0001 010004 020001 0001 03daaa 00 00800000 FE"_hex;
