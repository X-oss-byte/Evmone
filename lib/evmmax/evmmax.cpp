...
template <typename UintT>
UintT ModArith<UintT>::div(const UintT& x, const UintT& y) const noexcept
{
    return mul(x, inv(y));
}

template <>
uint256 ModArith<uint256>::inv(const uint256& x) const noexcept
{
...
template <>
uint384 ModArith<uint384>::inv(const uint384&) const noexcept
{
    assert(false); // FIXME: Implement
}

template <typename UintT>
UintT ModArith<UintT>::pow(const UintT& x, const UintT& y) const noexcept
{
    if (y == 0)
        return 1;
    else if (y == 1)
        ...
        return mul(pow(x, y / 2), mul(x, x));
    else
        return mul(mul(pow(x, y / 2), mul(x, x)), x);
}

template <typename UintT>
UintT ModArith<UintT>::neg(const UintT& x) const noexcept
{
    return sub(mod, x);
}
