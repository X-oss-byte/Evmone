...
public:
    ...
    {
        return v.size() - 1 - (size_t)std::distance(v.begin(),
            std::find_if(v.begin(), v.end(), [](auto v)  {return v != 0;}));
    }

    static inline constexpr std::vector<UintT> poly_rounded_div(const std::vector<UintT>& a,
        const std::vector<UintT>& b)
    {
        auto dega = deg(a);
        auto degb = deg(b);
        auto temp = a;
        auto o = std::vector<UintT>(a.size());
        for (size_t i = dega - degb; i >= 0; --i)
        {
            o[i] = arith.add(o[i], arith.div(temp[degb + i], b[degb]));
            for (size_t c = 0; c < degb + 1; ++c)
                temp[c + i] = arith.sub(temp[c + i], o[c]);
        }

        return std::vector<UintT>(o.begin(),
            o.begin() + (typename std::vector<UintT>::difference_type)deg(o) + 1);
    }

    static inline constexpr PolyExtFieldElem inv(const PolyExtFieldElem& x)
    {
        std::vector<UintT> lm(degree + 1); lm[0] = 1;
        ...
        while (deg(low) > 0)
        {
            ...
            assert(lm.size() == hm.size() == low.size() == high.size() == _new.size() == degree + 1);
    ...
    static inline constexpr PolyExtFieldElem div(const PolyExtFieldElem& x,
        const PolyExtFieldElem& y)
    {
        return mul(x, inv(y));
    }
    ...
    {
        std::vector<UintT> _one(degree); _one[0] = 1;
    ...
    static inline constexpr PolyExtFieldElem pow(const PolyExtFieldElem& x, UintT y)
    {
        if (y == 0)
            ...
            return mul(mul(x, x), pow(x, y / 2));
        else
            return mul(mul(mul(x, x), pow(x, y / 2)), x);
    }
